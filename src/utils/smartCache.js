...
function CacheInterface(nameOfDb) {
    ...
    this.saveToDB = (cohort, barcode, /*gene,*/ expression, payload) => {
        let dv = this.db
    ...
    this.getFromDB = (cohort, barcode, expression) => {
        this.db
            .getCollection(cohort)
            .data.map((expr) => Object.values(expr.barcodes));
    };

    this.add = (cohort, barcode, expression, payload) => {
        ...
        if (!expression && !payload) {
            ...
            interface
                .get(cohort)
                .set(expression, new Map([[barcode, payload]]));
        }
    };

    this.findDiff = (listOfBarcodes, cohort, expression) => {
        let s = new Set();
        let foundS = new Set();
        let interface = this.interface;
    ...
    this.constructQueries = (
        ...
        for (let cohort of listOfCohorts) {
            ...
            for (let expr of listOfExpressions) {
                ...
                let [diff, alreadyHave] = this.findDiff(
                    listOfBarcodes,
                    cohort,
                    expr
                );
                ...
                if (alreadyHave.size > 0) {
                    alreadyHave.forEach((val) =>
                        foundRes[cohort][expr].push(val)
                    );
                }
            }
        }
    ...
    this.fetchWrapperGE = async (
        listOfCohorts,
        listOfGenes,
        listOfBarcodes
    ) => {
        findDiff = (listOfBarcodes, cohort, expression, interface) => {
            let s = new Set();
            let foundS = new Set();
            if (
                interface.has(cohort) &&
                interface.get(cohort).has(expression)
            ) {
                let map = interface.get(cohort).get(expression);
                for (let barcode of listOfBarcodes) {
                    if (!map.has(barcode)) {
        ...
        const constructQueriesGE = (
            ...
            for (let cohort in res) {
                //If no genes are cached for a certain cohort at all, then delete the JSON field for cohort from res
                if (Object.keys(res[cohort]).length === 0) delete res[cohort];
            }
            return [res, foundRes];
        };
         ...
         * @returns
         */
        const executeQueriesGE = async (interface) => {
        ...
        let [missingInterface, hasInterface] = constructQueriesGE(
            listOfCohorts,
            listOfGenes,
            listOfBarcodes,
            this.interface
        ...
        for (let cohort in hasInterface) {
            ...
            let currentListOfBarcodes = await cacheBAR.fetchWrapperBAR(
    ...
    this.fetchWrapperMU = async (
         ...
         */
        const formatMutationData = async (cohort, gene, mutationData) => {
            ...
            if (mutationData != undefined) {
                ...
                for (let index = 0; index < mutationData.length; index++) {
                    ...
                    mutationData[index].Tumor_Sample_Barcode = mutationData[
                        index
                    ].Tumor_Sample_Barcode.substring(0, 12); // Get substring of tumor sample barcode to match TCGA participant barcode format
                    let barcodeInCohort = cohortBarcodes.includes(
                        mutationData[index].Tumor_Sample_Barcode
                    ); // Track whether current mutation data barcode is present in cohort's barcodes
                    if (!barcodeInCohort) {
                        ...
                        if (
                            barcodeMutations.has(
                                mutationData[index].Tumor_Sample_Barcode
                            )
                        ) {
                            let mutationsSet = barcodeMutations.get(
                                mutationData[index].Tumor_Sample_Barcode
                            ); // Get current set of mutations for current barcode
                            mutationsSet.add(
                                mutationData[index].Variant_Classification
                            ); // If current mutation data barcode is present, then append the mutation type to variants set
                            barcodeMutations.set(
                                mutationData[index].Tumor_Sample_Barcode,
                                mutationsSet
                            ); // Set current barcode's value to updated set of mutations
                        } else {
                            ...
                            barcodeMutations.set(
                                mutationData[index].Tumor_Sample_Barcode,
                                set
                            ); // Initialize the set of mutations for current barcode
                        }
                    }
                }
                ...
                barcodeMutations.forEach((mutationsSet, barcode) => {
                    ...
                    if (mutationsArr.length > 1) {
                        for (
                            let index = 1;
                            index < mutationsArr.length;
                            index++
                        )
                            mutationLabel += '_&_' + mutationsArr[index];
                    }
                    mutationDataToReturn.push({
         ...
         */
        const executeQueriesMU = async (interface) => {
            //Iterate over each cohort, gene combination
            for (let cohort in interface) {
                for (let gene of interface[cohort]) {
                    //Fetch expression data for requested cohort, gene, and barcodes with Firebrowse fetch call
                    let rawMutationData = await firebrowse.fetchMutationMAF({
                        cohorts: [cohort],
                        genes: [gene],
                    });
                    let mutationData = await formatMutationData(
                        cohort,
                        gene,
                        rawMutationData
                    );
                    for (let index = 0; index < mutationData.length; index++) {
                        try {
                            ...
                            await cacheMU.add(
                                ...
                                gene,
                                obj
                            ); // Add patient to mutation data caching interface
                            await cacheMU.saveToDB(
                                obj.cohort,
                                obj.tcga_participant_barcode,
                                gene,
                                obj
                            ); // Save patient to caching interface database
                        } catch (err) {
                            //If an error occurred, print an error message and end execution
                            console.error('Failed, skipping for cohort.', err);
                            return undefined;
            ...
            for (let gene of hasInterface[cohort]) {
                ...
                for (let index = 0; index < mutationData.length; index++) {
                    ...
                    if (!listOfBarcodes) {
                        ...
                        if (
                            listOfBarcodes.includes(
                                mutationData[index].tcga_participant_barcode
                            )
                        )
                            tmp.push(mutationData[index]); // Append patient to mutationData
                    }
        ...
        for (let cohort in missingInterface) {
            ...
            for (let gene of missingInterface[cohort]) {
                ...
                for (let index = 0; index < mutationData.length; index++) {
                    // If no barcodes to filter by have been specified, then append patient
                    if (!listOfBarcodes) tmp.push(mutationData[index]);
                    // Append patient to mutationData
                    else {
                        if (
                            listOfBarcodes.includes(
                                mutationData[index].tcga_participant_barcode
     ...
     */
    this.fetchWrapperBAR = async (listOfCohorts) => {
        /**
         ...
         */
        const constructQueriesBAR = (listOfCohorts, interface) => {
            let res = {};
            let foundRes = {};
            for (let cohort of listOfCohorts) {
         ...
         */
        const executeQueriesBAR = async (interface) => {
    ...
    this.fetchWrapperCLIN = async (listOfCohorts, barcodesByCohort) => {
        const constructQueriesCLIN = (listOfCohorts, interface) => {
            let res = {};
            let foundRes = {};
            for (let cohort of listOfCohorts) {
                if (!interface.has(cohort)) {
        ...
        const executeQueriesCLIN = async (barcodesByCohort, interface) => {
            for (let cohort in interface) {
                let getBarcodesInACohort = barcodesByCohort.filter(
                    (cohortEle) => cohortEle.cohort == cohort
                )[0].barcodes;
                let clinicalData = await firebrowse
                    .fetchClinicalFH({
                        cohorts: [cohort],
                    ...
                    .then((clinicalData) => {
                        ...
                        for (
                            ...
                            cacheCLIN.add(
                                (cohort = obj.cohort),
                                (barcode = obj)
                            ); // Add clinical data to interface map by mimicking parameters for barcode caching
                            cacheCLIN.saveToDB(
                                obj.cohort,
                                obj.tcga_participant_barcode,
                                obj
                            ); // Save clinical data to interface
                        }
                    })
                    .catch((error) => {
        ...
        for (let cohort in hasInterface) {
            ...
            let cohortOfInterest = barcodesByCohort.find(
                (obj) => obj.cohort == cohort
            );
...
CacheInterface.prototype.pprint = () => {
    const f = (entries) => {
