 ...
 */
const _fetchFromFireBrowse = async (endpoint, params, expectedKey) => {
    const proxy = 'https://corsproxy.io';
    const base = 'http://firebrowse.org/api/v1';

    // Add a leading / in the endpoint so we don't have / in the endpoint.
    ...
    try {
        const response = await axios.get(`${proxy}?${base}${endpoint}`, {
            params: params,
        });
        ...
        if (!json) {
            console.log(
                `${expectedKey} is empty, returning an object with empty ${expectedKey}`
            );
            return { [expectedKey]: [] };
        }
        ...
        console.error(
            `Fetching ${expectedKey} data was unsuccessful. Error:`,
            error
        );
        return { [expectedKey]: [] };
    }
 ...
 * //  }]
 */
const _paramsToParamsMatrix = (params, groupBy) => {
    const newParams = [];
    ...
    if (groupBy.length === 1) {
        ...
        for (let i = 0; i < params[groupByA.key].length; i += groupByA.length) {
            const slicedObject = {
                [groupByA.key]: params[groupByA.key].slice(
                    i,
                    i + groupByA.length
                ),
            };
        ...
        for (let i = 0; i < params[groupByA.key].length; i += groupByA.length) {
            for (
                ...
                const slicedObject = {
                    ...
                    [groupByB.key]: params[groupByB.key].slice(
                        j,
                        j + groupByB.length
                    ),
                };
            ...
            for (
                ...
                for (
                    ...
                    const slicedObject = {
                        ...
                        [groupByC.key]: params[groupByC.key].slice(
                            k,
                            k + groupByC.length
                        ),
                    };
 ...
 *    });
 */
firebrowse.fetch = async (endpoint, params, groupBy) => {
    ...
    if (groupBy == null || groupBy.length === 0) {
        ...
        for (let i = 0; i < paramsMatrix.length; i++) {
            ...
            const call = _fetchFromFireBrowse(
                endpoint,
                paramsForThisCall,
                expectedKey
 ...
 */
firebrowse.fetchClinicalFH = async function ({
    cohorts,
    genes,
    barcodes,
    pageNum,
    ...
    const data = await firebrowse.fetch(
        '/Samples/Clinical_FH',
        params,
        groupBy
    );
 ...
 */
firebrowse.fetchCounts = async (cohorts) => {
    if (!cohorts) {
...
firebrowse.fetchMutationMAF = async ({ cohorts, genes }) => {
    const params = {
        format: 'json',
        cohort: cohorts,
        tool: 'MutSig2CV',
 ...
 **/
firebrowse.fetchmRNASeq = async ({ cohorts, genes, barcodes }) => {
    if (!cohorts && !genes && !barcodes) {
        console.error('no arguments provided to function');
    }
    const params = {
